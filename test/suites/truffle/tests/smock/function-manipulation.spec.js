/* Imports: External */
const { toPlainObject, toArray } = require('lodash')
const { BigNumber } = require('ethers')

/* Imports: Internal */
const { smock } = require('../../../../../build/truffle')

contract('[hardhat] smock: function manipulation tests', () => {
  let mock
  beforeEach(async () => {
    mock = await smock('TestHelpers_BasicReturnContract')
    console.log('// function-manipulation.spec.js')
    console.log(`mock is: ${mock}`)
  })

  let FallbackCaller
  beforeEach(async () => {
    FallbackCaller = await artifacts.require('TestHelpers_FallbackCaller').new()
  ...
  describe('manipulating fallback functions', () => {
    it('should return with no data by default', async () => {
      ...
      expect(await FallbackCaller.doFallback.call()).to.equal(expected)
    })
    ...
    it('should be able to make a fallback function revert without any data', async () => {
      ...
      await expect(FallbackCaller.doFallback.call()).to.be.reverted
    })
    ...
    it('should be able to make a fallback function revert with a string', async () => {
      ...
      await expect(FallbackCaller.doFallback.call()).to.be.revertedWith(
        expected
      )
    })
